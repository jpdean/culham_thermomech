...
from utils import (
    ...
    create_mesh_tags_from_locators,
...
T_expr = TimeDependentExpression(
    lambda x, t: np.sin(np.pi * x[0]) * np.cos(np.pi * x[1]) * np.sin(np.pi * t)
)
...
volume_ids = {"volume_0": 8, "volume_1": 2}
boundary_ids = {}
boundary_ids["T"] = {"boundary_0": 9, "boundary_1": 5, "boundary_2": 6, "boundary_3": 8}
...
def get_material_mt(mesh):
    regions = {
        volume_ids["volume_0"]: lambda x: x[0] <= 0.5,
...
neumann_bc = TimeDependentExpression(
    lambda x, t: np.pi
    * (
        np.sin(x[0] * np.pi) ** 2 * np.sin(np.pi * t) ** 2 * np.cos(x[1] * np.pi) ** 2
        + 4.1
    )
    * np.sin(np.pi * t)
    * np.cos(x[0] * np.pi)
...
bcs = {}
bcs["T"] = {
    boundary_ids["T"]["boundary_0"]: {"type": "convection", "value": T_inf, "h": h},
    boundary_ids["T"]["boundary_1"]: {"type": "heat_flux", "value": neumann_bc},
    boundary_ids["T"]["boundary_2"]: {"type": "temperature", "value": T_expr},
    boundary_ids["T"]["boundary_3"]: {"type": "temperature", "value": T_expr},
}
bcs["u"] = {
...
def compute_f_u(T_expr, t_end, T_e, u_e, materials):
    ...
    return -ufl.div(
        thermomech.sigma(
            u_e,
            T_e,
            materials[volume_ids["volume_0"]]["thermal_strain"][1],
...
def test_temporal_convergence():
    ...
    for i in range(len(num_time_steps)):
        ...
        errors_L2["T"].append(compute_error_L2_norm(mesh.comm, results["T"], T_e))
        errors_L2["u"].append(compute_error_L2_norm(mesh.comm, results["u"], u_e))
